...
class Tetrahedron:
    def __init__(self, vertices, tet_num, depth=0):
        ...
        self.occupancy = torch.tensor(1. / 6000).requires_grad_()  # torch.rand(1)  # very small chance to all be 0
    ...
    def center(self, requires_grad=False, return_vertex=True):
        if requires_grad:
            a = torch.stack([v.curr_loc for v in self.vertices]).requires_grad_()
        else:
...
class QuarTet:
    ...
    def zero_grad(self):
        for tet in self.curr_tetrahedrons:
            tet.occupancy = tet.occupancy.detach().clone()
    ...
    def get_centers(self, requires_grad=False):
        centers = []
        for tet in self.curr_tetrahedrons:
            centers.append(tet.center(requires_grad, False))
    ...
    def get_occupied_centers(self):
        ...
        for tet in self.curr_tetrahedrons:
            if tet.occupancy >= 0.5:
                occupied_centers.append(tet.center().curr_loc)
...
if __name__ == '__main__':
    # a = QuarTet(2, 'cpu')
    # a = QuarTet('../quartet.tet', load_occupancies=True)
    a = QuarTet('../objects/cube_0.1.tet')
    a.export_metadata('../quartet_cache/cube_0.1_quartet_data.data')
    # a.fill_sphere()
    # for tet in a:
    #     tet.occupancy = torch.tensor(0.)
    ...
    # a = QuarTet()
    # a.load('initial_quartet.tet', 'cpu', 'default')
    a.export_mesh('../mesh.obj')
